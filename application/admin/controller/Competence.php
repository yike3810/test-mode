...
class Competence extends Admin
{
    public function index()
    {
        ...
        if ($keyword != "") {
            ...
            $where1['Cname'] = array("LIKE", "%$keyword%");
        }
//        a($where1['Cname'] );
...
//        a($sidlist);
    ...
    public function getCompetence_list()
    {
        parent::userauth2(12);
        $keyword = input('request.keywords');
        ...
        $where['Sid'] = array("EQ", 0);
        $where1['Sid'] = array("NEQ", 0);
        $sidlist = $competence->where($where1)->order('Dtime')->select();

        $lists = $competence->where($where)->paginate(4);
        $volist = $lists->toArray();
...
//            if (input('post.post') == 'ok') {
            ...
            if ($id == '' || !is_numeric($id)) {
                ...
                $id = intval($id);
                $competence = new \app\common\model\Competence;
                if ($competence->where(["ID"=>$id])->value('ID')) {
                    $competence->where(['ID'=> $id])->whereOr('Sid', $id)->delete();
