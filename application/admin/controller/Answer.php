...
class Answer extends Admin {
	...
	public function info() {
        ...
        foreach ($question_list as $k=>$v){
        	...
        	$question_list[$k]['answer_detail_info'] = $answer_detail->where(["question_id"=>$v['question_id'] ,"answer_id"=>$answer_id])->find();
		}
        //查出相应数据
        $exam = new \app\common\model\Exam;
...
// 			$data['answer_analysis']   	= input('post.answer_analysis');
			...
			$member 					=   new \app\common\model\Member;
			$question_info = $question->where(["question_title"=>$data['question_title'],"exam_id"=>$data['exam_id']])->find();
	...
	public function edit_do() {
		...
		if (request()->isPost()) {
			...
			$question_detail	=   new \app\common\model\Questiondetail;
			$member 	=   new \app\common\model\Member;
			$question_info = $question->where(["question_title" => $data['question_title'] ,"question_id" => ["neq",$data['question_id']]])->find();
			if(!empty($question_info)){
			    $this->error("该考试题目已存在!");
			}
	...
	public function save_field_do() {
		...
		if (request()->isPost()) {
			...
			if($field == "question_title"){
				$question_info = $question->where(["question_title" => $value, "question_id" =>["neq",$data['question_id']]])->find();
				if(!empty($question_info)){
