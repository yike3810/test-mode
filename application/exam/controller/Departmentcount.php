...
use think\Request;
use think\Url;
class Departmentcount extends Admin {
	public function index() {
		...
		$where = array();
		$where['exam_id']= input('request.exam_id');
		...
		$answer_list  = $answer->getDepartmentCountList($where);
		$lists   = $answer->paginate(config('paginate.list_rows'),$answer_list['total'],array('query'=>$where));
	...
	public function answer() {
		parent::userauth2(172);
	    $where = array();
	    $where['keywords']      = input('request.keywords');
	    $where['exam_id']		= input('request.exam_id');
	    $where['department_id']	= input('request.department_id');
	    ...
	    $this->assign('department_answer_list',$department_answer_list);
	    return $this->fetch();
	}
...
// 			$data['answer_analysis']   	= input('post.answer_analysis');
	...
	public function departmentexport() {
		...
		$objPHPExcel->getActiveSheet(0)->getStyle('A2:E2')->getFont()->setBold(true);      //第二行是否加粗
		...
		$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', "考试结果信息（{$volist['exam_name']}）");
		$ki = 2;
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$ki, '部门')
		->setCellValue('B'.$ki, '参与考试人数')
		->setCellValue('C'.$ki, '最高分')
		->setCellValue('D'.$ki, '最低分')
		->setCellValue('E'.$ki, '平均分');
		$sheet = $objPHPExcel->getActiveSheet();
		$objPHPExcel->getActiveSheet(0)->getStyle("E")->getAlignment()->setWrapText(TRUE);
		foreach ($answer_list['data'] as $k=>$v){
			...
			->setCellValueExplicit("B{$i}", $v['t'], \PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit("C{$i}", $v['max_points'], \PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit("D{$i}", $v['mix_points'], \PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValueExplicit("E{$i}", $v['avg_points'], \PHPExcel_Cell_DataType::TYPE_STRING);
		}
		...
		//$objPHPExcel->getActiveSheet(0)->getStyle("A1:E{$i}")->applyFromArray($styleThinBlackBorderOutline);
		// Add title
		$objPHPExcel->setActiveSheetIndex(0);
	...
	public function answerexport() {
	    ...
	    $member = new \app\common\model\Member;
	    $answer = new \app\common\model\Answer;
	    $where['list_rows'] = 66666;
	    $answer_list  = $answer->getDepartmentExamList($where);
	    $department_name = $department->where("id=".$where['department_id'])->value("department_name");
	    $exam_name = $exam->where("exam_id=".$where['exam_id'])->value("exam_name");
	    //$lists   = $answer->paginate(config('paginate.list_rows'),$answer_list['total'],array('query'=>$where));
	    /*数据查询,与列表页保持一致，结束*/
	    ...
	    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', "考试结果信息（{$exam_name}-{$department_name}）");
	    $ki = 2;
	    $objPHPExcel->setActiveSheetIndex(0)
	    ->setCellValue('A'.$ki, '部门')
